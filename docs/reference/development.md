...
CI runs the same gate in parallel: four `coverage-shards` matrix jobs execute their shard's manifest commands with `bun script/coverage-check.ts --execute-only` (commands only, no threshold evaluation) and upload every lcov report; the `Coverage` job downloads the reports and enforces the identical unioned thresholds with `--aggregate`, so CI verdicts match a local full run. `--package` accepts comma-separated lists for shard use, and `--aggregate` never runs commands — it fails when any manifest package lacks a report.
...
| Bug report drafting from a diagnosed Synergy problem                                             | `report-bug`                                       |
