...
A task with an invalid attachment URL remains in the inbox with `status: "failed"` and a failure reason. Runnable-work queries and startup discovery skip it while allowing later tasks to proceed. Retry reopens its payload-failed rollout before clearing the parked state; persistence errors propagate. Failed tasks remain removable and retryable before the first root materializes, cannot be guided, and are excluded from bulk guide actions. A rootless session containing only parked tasks reports the materialization failure without fabricating an assistant transcript.
