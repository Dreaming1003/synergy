...
Offline migration verifies historical trees in batches and publishes retention refs through a durable packed-ref file before switching ownership. Loose copies remain until the packed file and its directory entry are flushed; import keeps and legacy sources survive any publication failure. Pre-upgrade loose-object packing is an artifact-only maintenance operation under process ownership and the exclusive Scope lease. It never imports records, drops unknown objects, or edits a source already included in an interrupted SQL backup. See [the maintenance decision](../decisions/implemented/bug-fix/2026-09-16-lossless-legacy-snapshot-compaction.md).
